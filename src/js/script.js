...
function setActiveDay (dateObj) {
  try {
    ...
    const attributes = day.getAttribute('class') + ' active'
...
function setActiveClass (dateObj) {
  try {
    ...
    const attributes = lesson.getAttribute('class') + ' active'
...
function setPastTime (dateObj) {
  try {
    const pastTime = getPastTime(dateObj, getCurrentClass(dateObj))
    const startTime = document.createElement('div')
    startTime.className = 'past-time'
    startTime.textContent = formatTime(pastTime)

    const elem = getLessonsOfDay(dateObj)[getCurrentClass(dateObj)]
...
function setTimeLeft (dateObj) {
  try {
    const timeLeft = getTimeLeft(dateObj, getCurrentClass(dateObj))
...
function setProgress (dateObj) {
  try {
    ...
    const progress = getClassProgress(date, getCurrentClass(date), MILLISECS_IN_CLASS)
...
function isOddWeek (dateObj) { // return 1 (true) - odd, return 0 (false) - even
  const currentWeek = Math.ceil((dateObj.getTime() - FIRST_MONDAY) / MILLISECS_IN_WEEK)
...
function getCurrentClass (dateObj) {
  ...
  if (currentTime < 540 || currentTime > 1090) { // > 9:00 - 18:10 <
    return -1
  } else if (currentTime >= 540 && currentTime <= 620) { // < 9:00 - 10:20 >
    return 0
  } else if (currentTime >= 630 && currentTime <= 710) { // < 10:30 - 11:50 >
    return 1
  } else if (currentTime >= 740 && currentTime <= 820) { // < 12:20 - 13:40 >
    return 2
  } else if (currentTime >= 830 && currentTime <= 910) { // < 13:50 - 15:10 >
    return 3
  } else if (currentTime >= 920 && currentTime <= 1000) { // < 15:20 - 16:40 >
    return 4
  } else if (currentTime >= 1010 && currentTime <= 1090) { // < 16:50 - 18:10 >
    return 5
  }
}
